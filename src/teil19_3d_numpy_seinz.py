...
def drehen3D(objekt, winkelwerte):
    ...
    if wx:
        rotMatrix = np.array([[1, 0, 0], [0, cos(wx), -sin(wx)], [0, sin(wx), cos(wx)]])
        objekt = np.matmul(objekt, rotMatrix)
    if wy:
        rotMatrix = np.array([[cos(wy), 0, sin(wy)], [0, 1, 0], [-sin(wy), 0, cos(wy)]])
        objekt = np.matmul(objekt, rotMatrix)
    if wz:
        rotMatrix = np.array([[cos(wz), -sin(wz), 0], [sin(wz), cos(wz), 0], [0, 0, 1]])
        objekt = np.matmul(objekt, rotMatrix)
    return objekt
